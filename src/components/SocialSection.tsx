...
const cardValues = ["25+", "4-8", "24", "100%"];

function Card({ index, visible }: { index: number; visible: boolean }) {
  const { t } = useLanguage();
  const item = t.social.cards[index];
      ...
      <div className="relative z-10 flex flex-col justify-between h-full" style={{ gap: "8rem" }}>
        <div className="flex justify-between items-start">
          <div className="flex items-baseline gap-2">
            <span className="card-text text-[72px] font-sans font-normal tracking-tighter leading-none">{cardValues[index]}</span>
  ...
  return (
    <section className="py-16 md:py-24 bg-black relative z-20">
      ...
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-16" style={{ display: "flex", flexDirection: "column", gap: "110px" }}>
        <h2 className="text-[2rem] md:text-[52px] font-semibold text-left lg:text-center max-w-4xl lg:mx-auto leading-[1.2] font-sans">
          {t.social.heading}
        </h2>
